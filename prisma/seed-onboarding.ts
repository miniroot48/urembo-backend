...
async function seedOnboardingRequirements() {
  ...
  try {
    ...
    const summary = await Promise.all([
      ...
      prisma.onboardingRequirement.count({ where: { role: user_role.retailer } }),
      prisma.onboardingRequirement.count({ where: { role: user_role.manufacturer } }),
    ]);
    ...
    sampleRequirements.forEach((req, index) => {
      console.log(`   ${index + 1}. [${req.role}] ${req.label} (${req.fieldType}) ${req.isMandatory ? 'Required' : 'Optional'} ${req.isPaymentRelated ? '💰' : ''}`);
    });
